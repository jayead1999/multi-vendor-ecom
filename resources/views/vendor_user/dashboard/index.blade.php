...
<div class="page-body">
  <div class="container-xl">
    ...
    @php
    $latestKyc = App\Models\Kyc::where('user_id', auth()->id())->latest()->first();
    @endphp

    @if(!$latestKyc)
    <div class="alert alert-danger" role="alert">
      <h3 class="alert-title text-danger">KYC Verification Required</h3>
      <div class="text-secondary">Please submit your KYC information to complete your account setup.</div>
      <div class="mt-3">
        <a href="{{ route('vendor.kyc.index') }}" class="btn btn-danger">Submit KYC</a>
      </div>
    </div>
    @elseif($latestKyc->status === 'pending')
    <div class="alert alert-warning" role="alert">
      <h3 class="alert-title text-warning">KYC Status: Pending</h3>
      <div class="text-secondary">Your KYC application is currently under review. We will notify you once it's processed.</div>
    </div>
    @elseif($latestKyc->status === 'rejected')
    <div class="alert alert-danger" role="alert">
      <h3 class="alert-title text-danger">KYC Status: Rejected</h3>
      <div class="text-secondary">Your KYC application was rejected. <br><strong class="text-danger">Reason:</strong> {{ $latestKyc->rejected_reason }}</div>
      <div class="text-secondary mt-2">Please review the reason and re-submit your KYC application.</div>
      <div class="mt-3">
        <a href="{{ route('vendor.kyc.index') }}" class="btn btn-danger">Re-submit KYC</a>
      </div>
    </div>
    @endif
